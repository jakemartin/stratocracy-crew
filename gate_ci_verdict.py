...
WHY THIS EXISTS, AND IT IS NOT A STYLE PREFERENCE. `run.py --week1` EXITS 0 WHEN IT
CANNOT RUN THE GATE AT ALL. Measured 2026-08-20 on a shell with no C++ compiler on
PATH: the run printed `[stop] No usable C++ compiler on PATH`, then
`INTEGRATION GATE BLOCK — failing: T-INT-04`, and returned exit code 0. A CI job of
the shape `run: python run.py --week1` is therefore green on a run that asserted
...
WHAT IT GRADES, and all six must hold:
  ...
  6. INTEGRATION VERDICT `INTEGRATION GATE PASS` present, and neither BLOCK nor
                        SKIPPED. A SKIPPED integration gate says in its own words
                        that "nothing was checked and nothing is claimed" -- which
                        is honest, and is not a pass.
