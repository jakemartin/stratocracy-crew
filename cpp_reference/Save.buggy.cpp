// Stratocracy — save & replay format, part (a). PASS-1 VARIANT, deliberately wrong.
//
// Three defects, each a realistic first attempt rather than a typo:
//   A  loadSave() parses into the CALLER'S object and validates afterwards, so a
//      refused header has already overwritten the state it promised to leave alone.
//   B  checkHeader() compares only formatVersion and rulesCommit -- two of the four
//      fields §4.10's Version policy enumerates.
//   C  onlyKeys() tolerates an unknown key instead of refusing it.
//
// It exists so the gate can be shown to SEE these. Generated from Save.good.cpp by
...
bool onlyKeys(const Json& obj, const std::vector<std::string>& allowed,
    ...
    // DEFECT C: an unknown key is tolerated instead of refused.
...
SaveLoadResult checkHeader(const Save& s, const SaveHeaderExpectation& expect) {
    ...
    // DEFECT B: dataHash and scenarioHash are never compared.
    return accept();
}
...
SaveLoadResult loadSave(const std::string& text, const std::string& origin,
    ...
    // DEFECT A: parses straight into the caller's object, then validates.
    SaveLoadResult parsed = parseSave(text, origin, out);
    if (!parsed.ok) return parsed;

    SaveLoadResult header = checkHeader(out, expect);
