// Stratocracy — scenario file & validator implementation, PASS 1 (GDD §4.7 Stub 7).
//
// This file carries ONE plausible misreading of the document and is otherwise the
// pass-2 implementation. T-SCN-11 says "the OPPOSING seat's cheapest land path", and
// every other quantifier in this stub is read off `guidedOpening` -- T-SCN-06 even
// insists on the NAMED hex -- so the opposing route is minimised over that seat's
// `guidedOpening.infantry` alone. Q28 ruled the other way: the property guarded is a
// RACE, and a race does not care which Infantry wins it, so the minimisation ranges
// over EVERY CanCapture-row unit the opposing seat deploys.
//
...
ScenarioLoadResult validateScenario(const Scenario& s,
    ...
    // ---- T-SCN-11: NON-CONTENTION. The opposing seat's route runs from the hex its
    // own guidedOpening names -- the same NAMED-hex quantifier T-SCN-06 uses.
    // Bridges ARE permitted here (asymmetry (ii)).
    for (ScenarioLane& lane : r.lanes) {
        const int foe = SIDE_COUNT - 1 - lane.side;
        for (const ScenarioGuided& g : s.guided) {
            if (g.side != foe) continue;
            int cost = 0;
            if (!laneCost(s, terrain, g.infantry, lane.objective, true, cost)) continue;
            lane.opposingFound = true;
            lane.opposingCost  = cost;
            lane.opposingFrom  = g.infantry;
        }
    }
    for (const ScenarioLane& lane : r.lanes) {
