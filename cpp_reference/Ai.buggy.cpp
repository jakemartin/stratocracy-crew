// Stratocracy — baseline opponent AI, PASS-1 HALLUCINATION (deliberately wrong).
//
// This file exists to be blocked. Both bugs are the plausible reading an author
// reaches from §2.9 alone, without Q9's ruling and §2.9's exact wording in front
// of them:
//
//   BUG 1 — the losing-attack guard drops its second half. "Skip a strictly-losing
//           attack (the unit would die and trade down)" reads naturally as "do not
//           attack if the counter kills you", and that is the cautious, obvious
//           implementation. It is wrong: §2.9 joins two conditions, so an Infantry
//           that dies putting a large dent in a Tank is trading UP and must still
//           strike. The over-cautious AI refuses every sacrifice. Caught by T-AI-05.
//   BUG 2 — build ties break by the order §2.4's table PRINTS its units (Infantry,
//           Tank, Artillery, Recon), which is the order an author reads them in and
//           the order the UnitType enum pins. Q9 ruled the priority is
//           Infantry > Recon > Artillery > Tank -- ascending COST -- and §4.7's own
//           text warns in as many words that this is NOT the printed order. Caught
//           by T-AI-06.
//
// Both compile, both look reasonable, and both change what the shipping opponent
...
bool buildPriorityLess(const UnitDef& a, const UnitDef& b) {
    // BUG 2: §2.4's table order, which is also the UnitType enum order.
...
bool isStrictlyLosing(const AiState& s, const AiUnit& attacker, const AiUnit& defender) {
    ...
    // BUG 1: dying is treated as sufficient. The "and trades down" half never runs,
    // so a favourable sacrifice is refused along with a bad one.
    return true;
}
