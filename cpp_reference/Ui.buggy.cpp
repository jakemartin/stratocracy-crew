...
//
// This file carries plausible misreadings of spec/ui_spec.md and §4.7 Stub 8, and is
// expected to be BLOCKED. Five defects, every one a reading a careful implementer
...
//       reading Q14 refuses.
//
//   (3) T-UI-05 -- `incomePerTurn` is taken from `accrueIncome`, the function that
//       already computes income. It reads 0 on turn 1 by Q8(a), and Ruling G says the
//       field is the STANDING rate. Reusing the existing function is the obvious move
//       and it is wrong.
//
//   (4) T-UI-05 -- `isGuidedMarked` is derived from the unit's CURRENT hex rather
//       than its placement. Every other per-unit field reads current state, so this
//       one looks like it should too; the stub says in as many words that the mark
//       "does not move when the unit does", and beat 1a's whole content is that the
//       marked unit moves.
//
//   (5) T-UI-05 -- `spawnBlocked` is set equal to `buildWaiting`. A build that cannot
//       place is a build that waits, so the two read as the same fact; Ruling E
//       exists because they are not, and the case that separates them is a boxed-in
//       factory with nothing queued.
//
// DEFECTS (3) AND (4) ARE PLANTED IN THE PROJECTION'S DERIVATION HELPERS, and this is
// the case that decided how T-UI-05 is written. On the first pass the check called
// those same helpers, and BOTH DEFECTS PASSED THE INVARIANT: the projection and the
// check were wrong together, so clause (b) compared a value to itself. The fix is in
// pass 2 -- clause (b) restates each derivation inline, from the stub's words -- and
// with it these two now fail the invariant as well as the fixture assertions that
// caught them before ((d) on the standing rate, (e2) on the mark surviving a move).
// Defect (5) is planted in the projection alone and fails clause (b) either way.
//
...
// `isGuidedMarked`'s derivation, in one place so the projection and T-UI-05 clause
// (b) cannot drift apart -- the gate recomputes through this same function against a
// world it built itself, which is what makes a wrong derivation fail rather than be
// reproduced twice.
//
...
bool guidedMarkedFor(const UiWorld& w, const UiUnit& u) {
    ...
    for (const ScenarioGuided& g : *w.guided)
        // ---- DEFECT (4): keyed on the unit's CURRENT hex. Every other per-unit field
        // reads current state, so this reads as consistent with them. It unmarks the
        // marked Infantry the moment beat 1a asks it to move.
        if (g.side == u.side && hexEqual(g.infantry, u.hex)) return true;
    return false;
}
...
// ---------------------------------------------------------------------------
// derivations the snapshot and T-UI-05 share
// ---------------------------------------------------------------------------
...
int standingIncomeRate(const EconomyState& e, const std::vector<TerrainDef>& terrain,
                       int side) {
    // ---- DEFECT (3): reuse the function that already computes income rather than
    // restating the rate. It is the obvious move -- one rate, one implementation, no
    // second copy of §2.7 to drift -- and a copy is taken so that reading the field
    // cannot move the purse, which makes it look careful.
    //
    // It is still wrong. `accrueIncome` answers what a turn PAYS, and Q8(a) makes that
    // 0 on turn 1; Ruling G says this field is the rate the holdings CARRY, which on
    // turn 1 is precisely the number that function refuses to return.
    EconomyState probe = e;
    return accrueIncome(probe, terrain, side, /*turnNumber=*/1);
}
...
UiSnapshot buildUiSnapshot(const UiWorld& w) {
    ...
    // table flags as spawn points -- the same test §2.7's income and spawn rules use.
    {
        ...
        for (const Objective* o : facs) {
            ...
            // ---- DEFECT (5): a build that cannot place is a build that waits, so the
            // two fields read as the same fact and one is set from the other. Ruling E
            // exists because they are not the same: a boxed-in factory with NOTHING
            // queued is blocked and not waiting, and this cannot express it.
            v.spawnBlocked = v.buildWaiting;
