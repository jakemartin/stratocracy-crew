// Stratocracy — movement & pathfinding, PASS-1 HALLUCINATION (deliberately wrong).
//
// This file exists to be blocked. Its findPath is correct; its `reachable` computes
// the highlight as "every hex within hexDistance <= move" — the plausible-looking
// shortcut that ignores terrain cost entirely. It is the exact failure §2.5 names in
// advance: "the highlight is the real move set, NOT AN ESTIMATE."
//
// It passes a naive eyeball (the shape is a hex ring, the start is included, the
// numbers look sane) and fails T-MOVE-01 and T-MOVE-02 mechanically, because a
// Mountains hex 1 step away costs 3 and a Water hex is not enterable at all.
#include "Move.h"
...
std::vector<ReachEntry> reachable(const Board& board, const std::vector<TerrainDef>& terrain,
    ...
    (void)terrain;   // <-- THE BUG: terrain cost is never consulted at all.
    if (!board.valid() || board.index(start) < 0) return out;
    for (int row = 0; row < board.bounds.rows; ++row) {
        for (int col = 0; col < board.bounds.cols; ++col) {
            const Hex h = offsetToAxial(col, row);
            const int d = hexDistance(start, h);
            if (d > move) continue;
            const int idx = board.index(h);
            if (idx < 0 || board.occupant[idx] != OCCUPANT_NONE) continue;
            ReachEntry e;
            e.hex  = h;
            e.cost = d;          // "one step, one point" — true only on Plains
            out.push_back(e);
        }
    }
